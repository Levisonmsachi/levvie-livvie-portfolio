document.addEventListener('DOMContentLoaded', function() {
    ...
    // Animate skills when scrolling down
    const animateSkills = () => {
    ...
    contactForm.addEventListener('submit', (e) => {
        ...
        if (isValid) {
            // Show loading state when sending email 
            const submitBtn = contactForm.querySelector('button[type="submit"]');
            submitBtn.disabled = true;
            submitBtn.innerHTML = '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Sending...';

            // Send email via EmailJS to my email
            emailjs.sendForm(
...
document.getElementById("viewPostsBtn").addEventListener("click", function(event) {
    ...
    // Hide after 5 seconds by adding fade-out class
    setTimeout(() => {
        messageDiv.classList.add("fade-out");
        setTimeout(() => messageDiv.classList.add("d-none"), 500);
    }, 5000);
});